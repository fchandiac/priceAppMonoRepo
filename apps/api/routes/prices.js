...
router.post('/prices/filter', (req, res) => {
    ...
    getPricesWithProductName(code, name, commerce, startDate, endDate, (err, filteredPrices) => {
        if (err) {
            res.status(500).json({ error: 'Error fetching filtered prices' });
            return;
        }
        ...
        // Obtener los precios de la página actual con paginación usando slice
